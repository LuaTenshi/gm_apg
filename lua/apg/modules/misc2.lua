...
local collisionCheckTimeFrame = 5
...
local function collcall(ent, data)
    ...
    if IsValid(ent) and IsValid(hit) and IsValid(mep) then
        ...
        obj.CollisionTime = obj.CollisionTime or (CurTime() + collisionCheckTimeFrame)
        obj.LastCollision = CurTime()

        -- TODO: New config var for this magic number
        local serverTickRate = math.Round(1/engine.TickInterval())
        local totalTicksInTimeFrame = serverTickRate * collisionCheckTimeFrame

        local collisionLimit = totalTicksInTimeFrame / 2

        if obj.Collisions > collisionLimit then
            obj.Collisions = 0
